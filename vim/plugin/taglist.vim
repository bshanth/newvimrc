...
if !exists('loaded_taglist')
	...
	nmap <silent> gf :silent! TlistToggle <CR> :exe 'wincmd p' <CR>

    " Mark the buffer as not modifiable
    "command! -nargs=0 -bar Tf TlistToggle
...
function! s:Tlist_Window_Display_Help()
	...
	"Addtheextensivehelp
	"callappend(0,'"<enter>:Jumptotagdefinition')
	"callappend(1,'"o:Jumptotagdefinitioninnewwindow')
	"callappend(2,'"p:Previewthetagdefinition')
	"callappend(3,'"<space>:Displaytagprototype')
	"callappend(4,'"u:Updatetaglist')
	"callappend(5,'"s:Selectsortfield')
	"callappend(6,'"d:Removefilefromtaglist')
	"callappend(7,'"x:Zoom-out/Zoom-intaglistwindow')
	"callappend(8,'"+:Openafold')
	"callappend(9,'"-:Closeafold')
	"callappend(10,'"*:Openallfolds')
	"callappend(11,'"=:Closeallfolds')
	"callappend(12,'"[[:Movetothestartofpreviousfile')
	"callappend(13,'"]]:Movetothestartofnextfile')
	"callappend(14,'"q:Closethetaglistwindow')
	"callappend(15,'"<F1>:Removehelptext')
    endif
endfunction
...
function! s:Tlist_Window_Init()
    ...
    if has('gui_running') || &t_Co > 2
        ...
        if hlexists('MyTagListTagName')
            ...
            highlight default link TagListTagName Ignore
        endif
        ...
        if hlexists('MyTagListTitle')
            ...
            highlight default link TagListTitle Ignore
        endif
...
"        highlight default TagListTagName term=reverse cterm=reverse
    ...
    nnoremap <buffer> <silent> <CR>
                \ :call <SID>Tlist_Window_Jump_To_Tag('useopen') <bar> wincmd p <bar> close <CR>
...
function! s:Tlist_Get_Tag_Prototype(fidx, tidx)
    ...
    if strlen(tproto_var) > g:Tlist_WinWidth
    echo tproto_var
    echo g:Tlist_WinWidth
        let g:Tlist_WinWidth = strlen(tproto_var)
    endif
