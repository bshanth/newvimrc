...
function! s:SetTagDisplay()
	...
	let s:title_tag_name = l:tag_name
        "let &titlestring=s:title_tag_name
	let &titlestring='%F%( %M%)%( (%{expand("%:~:.:h")})%)%( %a%)%='
   "     let tlist = taglist(s:title_tag_name)
   "     let tdict = get(tlist,0)
    
"        if (g:qfnextflag == 2)
"            let s:new_title_tag_name = s:title_tag_name."clmode"
"        elseif (g:qfnextflag == 1)
"            let s:new_title_tag_name = s:title_tag_name."cscopemode"
"        else
"            let s:new_title_tag_name = s:title_tag_name."pmode"
"        endif

        exe ":set tabline=".s:title_tag_name
    endif
endfunction
